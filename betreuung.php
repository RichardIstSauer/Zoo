...
$query = "SELECT einsatz.e_id, revier.reviername, pfleger.vorname, pfleger.nachname FROM revier, pfleger, einsatz WHERE revier.r_id=einsatz.r_id AND einsatz.pf_id=pfleger.pf_id;";
...
for ($i = 0; $i < $count; $i++) {
  ...
  echo "<td><a href='?delete=$einsatzID[$i]' class='btn btn-danger'>Löschen</a></td>";
  echo "</tr>";
}
echo "</table>";
...
<form method="post">
  ...
  <select class="form-control" name="pflegerSelect">
    <?php
...
if (isset($_GET["delete"])) {
  $id = $_GET["delete"];
  $sql = "DELETE FROM einsatz WHERE e_id=$id";
  if ($conn->query($sql) == FALSE) {

    echo "Fehler beim Löschen: " . $conn->error;
  } else {
    echo '<meta http-equiv="refresh" content="0;url=betreuung.php" />';
  }
}
